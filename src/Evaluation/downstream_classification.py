...
def wrn_classify(train_set, test_set, fp, num_classes=10):
    batch_size = 512
    lr = 0.1
    max_epoch = 50
    criterion = nn.CrossEntropyLoss()
    ...
    test_loader = DataLoader(test_set, shuffle=False, batch_size=batch_size, drop_last=False, num_workers=2)
    ...
    for epoch in range(max_epoch): 
        for _, (inputs, targets) in enumerate(train_loader):
            ...
            optimizer.step()
        scheduler.step()
    torch.save(model.state_dict(), os.path.join(fp, "trained_wrn_weight.pth"))
    model.eval()
    total = 0
    correct = 0
    with torch.no_grad():
...
def main(args):
    ...
    if args.dataset == "cifar10":
        test_set = CIFAR10(root=args.test_dir, train=False, transform=transform_test)
        num_classes = 10
    else:
    ...
    if args.model == "cnn":
        ...
        acc = wrn_classify(train_set, test_set, args.out_dir, num_classes)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, default="logreg", choices=["logreg", "mlp", "cnn", "wrn"])

    args = parser.parse_args()
    model_list = ["logreg", "mlp", "cnn"]
