...
def edm_sampler(x, y, denoiser, num_steps, tmin, tmax, rho, bound=1e5, guid_scale=None, s_noise=1, s_churn=0, s_min=0, s_max=float('inf'), churn_limit=1, **kwargs):
...
def ddim_sampler(x, y, denoiser, num_steps, tmin, tmax, rho, bound=1e5, guid_scale=None, stochastic=False, **kwargs):
    t_steps = torch.linspace(tmax ** (1. / rho), tmin **
                             (1. / rho), steps=num_steps, device=x.device) ** rho
    x = x * t_steps[0]
    ...
    # norm = (x.view(x.shape[0], -1) ** 2).sum(dim=1).sqrt()
    # print(norm)
    # reject_idx = norm > bound
    # x[reject_idx] = x[reject_idx] / norm[reject_idx].view(-1, 1, 1, 1) * bound
